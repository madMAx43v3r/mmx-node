...
void recursive_sha256_ni(uint8_t* hash, const uint64_t num_iters)
{
	...
	// optimization: rewrite with prerequisite and all code inside function

	alignas(64)
	const uint32_t localK[] = {
		0x428A2F98, 0x71374491, 0xB5C0FBCF, 0xE9B5DBA5,
		0x3956C25B, 0x59F111F1, 0x923F82A4, 0xAB1C5ED5,
		0xD807AA98, 0x12835B01, 0x243185BE, 0x550C7DC3,
		0x72BE5D74, 0x80DEB1FE, 0x9BDC06A7, 0xC19BF174,
	...
	for(uint64_t i = 0; i < num_iters; ++i) {
		...
		const uint64_t num_bits = 0x0001000000000000; // (CHANGED/PRECALC) const uint64_t num_bits = bswap_64(length * 8);
		const uint8_t end_bit = 0x80;

		uint8_t last[128] = {};
		::memcpy(last, hash, 32); // (CHANGED/PRECALC) ::memcpy(last, in + num_blocks * 64, remain);
		...
		::memcpy(last + 56, &num_bits, 8); // (CHANGED/PRECALC) ::memcpy(last + final_blocks * 64 - 8, &num_bits, 8);

		// start: compress_digest()

		const __m128i* K_mm = reinterpret_cast<const __m128i*>(localK);

		const __m128i* input_mm = reinterpret_cast<const __m128i*>(last); // (CHANGED/PRECALC) const __m128i* input_mm = reinterpret_cast<const __m128i*>(input);
		const __m128i MASK = _mm_set_epi64x(0x0c0d0e0f08090a0b, 0x0405060700010203);
		...
		for(int k = 0; k < 8; ++k) {
			// (CHANGED/PRECALC) state[k] = bswap_32(state[k]);
			const uint32_t val = state[k];
			state[k] = ((val & 0xFF) << 24) | ((val & 0xFF00) << 8) | ((val & 0xFF0000) >> 8) | ((val & 0xFF000000) >> 24);
		}
