...
uint64_t NodeAsyncClient::get_txo_info(const ::mmx::txio_key_t& key, const std::function<void(const ::mmx::txo_info_t&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
uint64_t NodeAsyncClient::get_history_for(const std::vector<::mmx::addr_t>& addresses, const uint32_t& min_height, const std::function<void(const std::vector<::mmx::tx_entry_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
		...
		case 16: {
			...
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::mmx::Node_get_txo_info_return>(_value)) {
					...
					_callback(_value->get_field_by_index(0).to<::mmx::txo_info_t>());
				} else {
					throw std::logic_error("NodeAsyncClient: invalid return value");
				}
			}
