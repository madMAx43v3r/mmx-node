...
uint64_t NodeAsyncClient::get_address_infos(const std::vector<::mmx::addr_t>& addresses, const int32_t& since, const std::function<void(const std::vector<::mmx::address_info_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
	...
	const auto _request_id = ++vnx_next_id;
	{
		...
		vnx_queue_get_address_infos[_request_id] = std::make_pair(_callback, _error_callback);
	}
	vnx_request(_method, _request_id);
	return _request_id;
}

uint64_t NodeAsyncClient::get_virtual_plots(const std::vector<::mmx::addr_t>& addresses, const std::function<void(const std::vector<::mmx::virtual_plot_info_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
int32_t NodeAsyncClient::vnx_purge_request(uint64_t _request_id, const vnx::exception& _ex) {
	...
	switch(_index) {
		...
		case 43: {
			const auto _iter = vnx_queue_get_address_infos.find(_request_id);
			if(_iter != vnx_queue_get_address_infos.end()) {
				const auto _callback = std::move(_iter->second.second);
				vnx_queue_get_address_infos.erase(_iter);
				_lock.unlock();
				if(_callback) {
					_callback(_ex);
				}
...
int32_t NodeAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	...
	switch(_index) {
		...
		case 43: {
			...
			_lock.unlock();
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::mmx::Node_get_address_infos_return>(_value)) {
					_callback(_result->_ret_0);
				} else if(_value && !_value->is_void()) {
		...
		case 44: {
			...
			_lock.unlock();
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::mmx::Node_get_virtual_plots_return>(_value)) {
					_callback(_result->_ret_0);
				} else if(_value && !_value->is_void()) {
