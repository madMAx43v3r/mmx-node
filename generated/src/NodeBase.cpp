...
const vnx::Hash64 NodeBase::VNX_CODE_HASH(0x60b2d9495af891eull);
	...
	:	Module::Module(_vnx_name)
{
	...
	vnx::read_config(vnx_name + ".reload_height", reload_height);
...
void NodeBase::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[15], 15); vnx::accept(_visitor, reload_height);
	_visitor.type_field(_type_code->fields[16], 16); vnx::accept(_visitor, num_sync_retries);
	_visitor.type_field(_type_code->fields[17], 17); vnx::accept(_visitor, do_sync);
	_visitor.type_field(_type_code->fields[18], 18); vnx::accept(_visitor, storage_path);
	_visitor.type_field(_type_code->fields[19], 19); vnx::accept(_visitor, router_name);
...
void NodeBase::write(std::ostream& _out) const {
	...
	_out << ", \"reload_height\": "; vnx::write(_out, reload_height);
...
vnx::Object NodeBase::to_object() const {
	...
	_object["reload_height"] = reload_height;
...
void NodeBase::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		...
		} else if(_entry.first == "reload_height") {
			_entry.second.to(reload_height);
...
vnx::Variant NodeBase::get_field(const std::string& _name) const {
	...
	if(_name == "reload_height") {
		return vnx::Variant(reload_height);
	}
...
void NodeBase::set_field(const std::string& _name, const vnx::Variant& _value) {
	...
	} else if(_name == "reload_height") {
		_value.to(reload_height);
...
std::shared_ptr<vnx::TypeCode> NodeBase::static_create_type_code() {
	...
	type_code->code_hash = vnx::Hash64(0x60b2d9495af891eull);
...
void read(TypeInput& in, ::mmx::NodeBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	if(type_code->is_matched) {
		...
		if(const auto* const _field = type_code->field_map[15]) {
			vnx::read_value(_buf + _field->offset, value.reload_height, _field->code.data());
		}
	...
	for(const auto* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			...
			case 18: vnx::read(in, value.storage_path, type_code, _field->code.data()); break;
			case 19: vnx::read(in, value.router_name, type_code, _field->code.data()); break;
...
void write(TypeOutput& out, const ::mmx::NodeBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	vnx::write_value(_buf + 12, value.reload_height);
	vnx::write_value(_buf + 16, value.num_sync_retries);
	vnx::write_value(_buf + 20, value.do_sync);
	...
	vnx::write(out, value.storage_path, type_code, type_code->fields[18].code.data());
	vnx::write(out, value.router_name, type_code, type_code->fields[19].code.data());
}
