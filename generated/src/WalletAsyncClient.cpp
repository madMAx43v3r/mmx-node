...
uint64_t WalletAsyncClient::get_balance(const uint32_t& index, const ::mmx::addr_t& currency, const uint32_t& min_confirm, const std::function<void(const uint64_t&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
uint64_t WalletAsyncClient::get_balances(const uint32_t& index, const uint32_t& min_confirm, const std::function<void(const std::map<::mmx::addr_t, uint64_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
...
uint64_t WalletAsyncClient::get_reserved_balances(const uint32_t& index, const uint32_t& min_confirm, const std::function<void(const std::map<::mmx::addr_t, uint64_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
	...
	const auto _request_id = ++vnx_next_id;
	{
		...
		vnx_queue_get_reserved_balances[_request_id] = std::make_pair(_callback, _error_callback);
	}
	vnx_request(_method, _request_id);
	return _request_id;
}
...
uint64_t WalletAsyncClient::get_accounts(const std::function<void(const std::map<uint32_t, ::mmx::account_t>&)>& _callback, const std::function<void(const vnx::exception&)>& _error_callback) {
	auto _method = ::mmx::Wallet_get_accounts::create();
	const auto _request_id = ++vnx_next_id;
	{
		std::lock_guard<std::mutex> _lock(vnx_mutex);
		vnx_pending[_request_id] = 31;
		vnx_queue_get_accounts[_request_id] = std::make_pair(_callback, _error_callback);
	}
	vnx_request(_method, _request_id);
	return _request_id;
}
...
int32_t WalletAsyncClient::vnx_purge_request(uint64_t _request_id, const vnx::exception& _ex) {
	...
	switch(_index) {
		...
		case 27: {
			const auto _iter = vnx_queue_get_reserved_balances.find(_request_id);
			if(_iter != vnx_queue_get_reserved_balances.end()) {
				const auto _callback = std::move(_iter->second.second);
				vnx_queue_get_reserved_balances.erase(_iter);
				_lock.unlock();
				if(_callback) {
					_callback(_ex);
				}
		...
		case 31: {
			const auto _iter = vnx_queue_get_accounts.find(_request_id);
			if(_iter != vnx_queue_get_accounts.end()) {
				const auto _callback = std::move(_iter->second.second);
				vnx_queue_get_accounts.erase(_iter);
				_lock.unlock();
				if(_callback) {
					_callback(_ex);
				}
...
int32_t WalletAsyncClient::vnx_callback_switch(uint64_t _request_id, std::shared_ptr<const vnx::Value> _value) {
	...
	switch(_index) {
		...
		case 27: {
			...
			_lock.unlock();
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::mmx::Wallet_get_reserved_balances_return>(_value)) {
					_callback(_result->_ret_0);
				} else if(_value && !_value->is_void()) {
					_callback(_value->get_field_by_index(0).to<std::map<::mmx::addr_t, uint64_t>>());
				} else {
		...
		case 31: {
			...
			_lock.unlock();
			if(_callback) {
				if(auto _result = std::dynamic_pointer_cast<const ::mmx::Wallet_get_accounts_return>(_value)) {
					_callback(_result->_ret_0);
				} else if(_value && !_value->is_void()) {
