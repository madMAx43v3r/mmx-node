...
const vnx::Hash64 HarvesterBase::VNX_CODE_HASH(0x37fd44b8a6008f2cull);
	...
	:	Module::Module(_vnx_name)
{
	...
	vnx::read_config(vnx_name + ".self_test", self_test);
}
...
void HarvesterBase::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[2], 2); vnx::accept(_visitor, plot_dirs);
	_visitor.type_field(_type_code->fields[3], 3); vnx::accept(_visitor, farmer_server);
	...
	_visitor.type_field(_type_code->fields[7], 7); vnx::accept(_visitor, self_test);
...
void HarvesterBase::write(std::ostream& _out) const {
	...
	_out << ", \"self_test\": "; vnx::write(_out, self_test);
	_out << "}";
}
...
void HarvesterBase::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		...
		} else if(_entry.first == "self_test") {
			_entry.second.to(self_test);
		}
	}
}
...
std::shared_ptr<vnx::TypeCode> HarvesterBase::static_create_type_code() {
	...
	type_code->code_hash = vnx::Hash64(0x37fd44b8a6008f2cull);
...
void read(TypeInput& in, ::mmx::HarvesterBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	if(type_code->is_matched) {
		...
		if(const auto* const _field = type_code->field_map[7]) {
			vnx::read_value(_buf + _field->offset, value.self_test, _field->code.data());
		}
	}
	for(const auto* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			...
			case 2: vnx::read(in, value.plot_dirs, type_code, _field->code.data()); break;
			case 3: vnx::read(in, value.farmer_server, type_code, _field->code.data()); break;
...
void write(TypeOutput& out, const ::mmx::HarvesterBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	vnx::write_value(_buf + 12, value.self_test);
	...
	vnx::write(out, value.plot_dirs, type_code, type_code->fields[2].code.data());
	vnx::write(out, value.farmer_server, type_code, type_code->fields[3].code.data());
}
