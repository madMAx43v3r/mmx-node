...
const vnx::Hash64 Delegated::VNX_CODE_HASH(0x84ff257ee2926e96ull);
...
void Delegated::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[6], 6); vnx::accept(_visitor, delegate_cond);
...
void Delegated::write(std::ostream& _out) const {
	...
	_out << ", \"delegate_cond\": "; vnx::write(_out, delegate_cond);
	_out << "}";
}
...
vnx::Object Delegated::to_object() const {
	...
	_object["user"] = user;
	_object["delegate_cond"] = delegate_cond;
	return _object;
}

void Delegated::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		if(_entry.first == "delegate_cond") {
			_entry.second.to(delegate_cond);
...
vnx::Variant Delegated::get_field(const std::string& _name) const {
	...
	if(_name == "delegate_cond") {
		return vnx::Variant(delegate_cond);
	}
	return vnx::Variant();
}
...
void Delegated::set_field(const std::string& _name, const vnx::Variant& _value) {
	if(_name == "version") {
		...
		_value.to(user);
	} else if(_name == "delegate_cond") {
		_value.to(delegate_cond);
	} else {
...
std::shared_ptr<vnx::TypeCode> Delegated::static_create_type_code() {
	...
	type_code->code_hash = vnx::Hash64(0x84ff257ee2926e96ull);
...
void read(TypeInput& in, ::mmx::contract::Delegated& value, const TypeCode* type_code, const uint16_t* code) {
	...
	for(const auto* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			...
			case 6: vnx::read(in, value.delegate_cond, type_code, _field->code.data()); break;
...
void write(TypeOutput& out, const ::mmx::contract::Delegated& value, const TypeCode* type_code, const uint16_t* code) {
	...
	vnx::write(out, value.delegate_cond, type_code, type_code->fields[6].code.data());
}
