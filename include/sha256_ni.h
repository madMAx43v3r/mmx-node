...
inline uint32_t h_bswap_32(const uint32_t val) {
	return ((val & 0xFF) << 24) | ((val & 0xFF00) << 8) | ((val & 0xFF0000) >> 8) | ((val & 0xFF000000) >> 24);
}

inline uint64_t h_bswap_64(const uint64_t val) {
	return (uint64_t(h_bswap_32(val)) << 32) | h_bswap_32(val >> 32);
}

inline void recursive_sha256_ni(uint8_t* hash, const uint64_t num_iters)
{
	...
	for(uint64_t i = 0; i < num_iters; ++i) {
		...
		// start: sha256_ni() optimize
        	...
        	const uint64_t num_bits = h_bswap_64(256);
			...
			const __m128i* K_mm = reinterpret_cast<const __m128i*>(h_K);
