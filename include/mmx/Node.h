...
protected:
	...
	vnx::Variant call_contract(	const addr_t& address, const std::string& method, const std::vector<vnx::Variant>& args = {},
								const vnx::optional<addr_t>& user = nullptr, const vnx::optional<std::pair<addr_t, uint80>>& deposit = nullptr) const override;
	...
	std::vector<offer_data_t> get_recent_offers_for(
			const vnx::optional<addr_t>& bid, const vnx::optional<addr_t>& ask, const uint80& min_bid,
	...
	std::array<uint128, 2> get_swap_trade_estimate(const addr_t& address, const uint32_t& i, const uint80& amount, const int32_t& num_iter) const override;
