...
articlePublished: 2026-03-01
articleUpdated: 2026-03-01
...
Nearly all plots will be within a 99.5% GiB range, with average plot size in middle.
Given enough plots there will be a few outliers, but not many.
99.5% is a representative value for a practical range.
GiB numbers with one decimal place have been rounded down and up to best fit purpose.
Less chance of underestimating average plot sizes and range values.
...
Let's take a 18TB disk. Convert free space to GiB, with 2 GiB filesystem overhead:
> `(18E12 / 1024^3) - 2.0` = `16761.8 GiB`

_TBD: Writing in progress (will only illustrate that not one perfect way to do it) ..._
